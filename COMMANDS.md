...
| `!setpin !name on\|off` | mod | Toggle auto-pin: when on, every send of that command by a moderator (manual trigger or timer auto-post) pins the sent message via Twitch's chat pin, staying pinned until the stream ends (replacing any message already pinned). A non-mod's manual `!name` trigger on a pin-enabled command still sends the text — it just doesn't pin it, since only a mod-triggered send performs the pin (a non-mod repeatedly triggering it can't be allowed to keep changing what's pinned). Use `modOnly` (below) instead if you want the whole command restricted to mods. A command can't have `timer` and `pin` on at the same time — `!settimer`/`!setpin` reject the change with an explanation if the other attribute is already active; disable it first. |
...
| `!name` | all (mod-only if `modOnly` is on for that command; a pin-enabled command still triggers for anyone, it just only pins when a mod triggered it) | Trigger a custom command (`#counterName` inside its text is replaced with the counter's live value). |

Three more per-command settings exist but are **web-panel-only** — there's no chat command for any of them, same as the announcement color (`TwitchBot-Web`'s `/<channel>/commands` page):
...
- **Text by stream category**: a command can have different response text for specific Twitch stream categories (e.g. a different `!совет`-style response while playing "Dota 2" vs. anything else), falling back to the command's plain text outside those categories. Matched case-insensitively against the category tracked by `twitch/streamStatus.js`. Unlike `!совет`'s Dota-2 gate (which just reads the up-to-5-minutes-stale poll cache), a command with category text set forces a fresh Get Streams check (`streamStatus.refreshCategory()`) on every manual trigger and auto-send, so switching category mid-stream is picked up immediately instead of on the next poll.
- **Moderators only (`modOnly`)**: when on, only mods can trigger the command manually with `!name` at all (non-mods' `!name` is silently ignored) — auto-posts are unaffected, since there's no triggering user to gate. This is the only thing that gates *triggering* — `pin` and `announce` don't (a pin-enabled command sends for everyone and only pins for mods; announce-enabled sends for everyone too). A mod has to opt a command into `modOnly` explicitly if they want it restricted.
