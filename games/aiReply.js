...
const SYSTEM_RULES = [
  ...
  '- Рядом с отношением показано, сколько раз этого человека уже прощали. Тот, кого простили и кто вернулся к тому же самому, извиняется не всерьёз, и повод отказать у тебя есть.',
...
function describePardons(relation, at = new Date()) {
  ...
  if (!count) return '';
  const times = count === 1 ? 'один раз' : count + ' раза';
