    ...
    properties: {
      ...
      selfTimeout: {
        ...
        description:
          ...
          'Просьба замутить кого-то другого сюда не идёт никогда. Пустая строка - не просил.',
      },
      pardon: {
        type: 'string',
        description:
...
const SYSTEM_RULES = [
  ...
  '- Просьба замутить кого-то ДРУГОГО — это не оно. Чужой тайм-аут ты не выдаёшь ни по чьей просьбе, даже если просит модератор: для этого у него есть свои кнопки.',
  '',
  'Память канала:',
...
const FIELD_PROTOCOL = [
  ...
  '- selfTimeout — число секунд, если зритель просит тайм-аут САМ СЕБЕ. Пусто во всех остальных случаях, и просьба замутить другого человека сюда не идёт никогда. Выдаётся вдвое больше запрошенного, отношение за такую просьбу не понижается, а ответ уходит в чат как обычно.',
...
async function answer(client, channel, userState, message, cfg, settings, scripted) {
  ...
  } else if (verdict === 'filter' && trusted && !selfSeconds) {
